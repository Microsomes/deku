...
let rec build_spam ~ticketer rounds_left ((batch_size, batch_count) as info)
    transactions block_level =
  if rounds_left = 0 then
    List.flatten transactions
  else
    let batch =
      List.init batch_count (fun _ ->
          make_transactions ~ticketer ~n:batch_size ~block_level ()) in
    build_spam ~ticketer (rounds_left - 1) info (batch :: transactions)
      block_level

let send_spam transactionsl =
  Lwt_list.iter_s
    (fun transactions -> spam_transactions ~transactions ())
    transactionsl
...
let load_test_transactions ticketer =
  ...
  let sending_block_level = 1L in
  let transactions =
    build_spam ~ticketer rounds (batch_size, batch_count) [] sending_block_level
  in
  let final_transaction = build_spam ~ticketer 1 (1, 1) [] sending_block_level in
  let final_transaction_hash =
    List.hd final_transaction |> List.hd |> fun transaction ->
    transaction.Protocol.Operation.Core_user.hash in
  Format.eprintf "transactions are built";
  let%await starting_block_level = get_current_block_level () in
  let%await _ = send_spam transactions in
  let%await final_block_level =
    get_last_block_height final_transaction_hash starting_block_level in
  let tps_period =
