...
let vm_state = Deku_external_vm.External_vm_protocol.State.empty

let sender_chain =
  Chain.make ~identity:identity1 ~validators ~default_block_size ~vm_state

let receiver_chain =
  Chain.make ~identity:identity2 ~validators ~default_block_size ~vm_state

let backup_chain1 =
  Chain.make ~identity:identity3 ~validators ~default_block_size ~vm_state

let backup_chain2 =
  Chain.make ~identity:identity4 ~validators ~default_block_size ~vm_state
